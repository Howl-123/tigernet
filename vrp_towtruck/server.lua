...
function isPlayerTowTrucker(user_id)
	if(vRP.hasPermission({user_id, towPermission}))then
		return true
	else
		return false
	end
...
local function ch_impoundVehicle(player, choice)
	...
	vRPclient.notify(player, {"Du solgte bilen for "..impoundFee.." DKK"})
end
	...
	if user_id ~= nil then
		vRPclient.getNearestVehicle(player, {7}, function(veh)
			if(veh)then
				...
				vRPclient.notify(player, {"Intet køretøj nær dig"})
			end
		end)
	end
end
...
local function ch_towMenu(player, choice)
	vRP.buildMenu({"Tow Truck Company", {player = player}, function(menu)
		menu.name = "Mekaniker menu"
		menu.css={top="75px",header_color="rgba(0,255,213,0.75)"}
		menu.onclose = function(player) vRP.openMainMenu({player}) end -- nest menu
		local user_id = vRP.getUserId({player})
		if(isTowing[user_id] == nil)then
			menu["Tow Vehicle"] = {ch_towVehicle,"Tow det nærmeste køretøj"}
		else
		menu["Release Vehicle"] = {ch_unimpoundVehicle,"Release køretøjet fra platformen"}
		end
...
local function impoundVehicle(player)
	...
	if(isTowing[user_id] ~= nil)then
		vRP.buildMenu({"Tow Vehicle", {player = player}, function(menu2)
			menu2.name = "Sælg køretøj"
			menu2.css={top="75px",header_color="rgba(0,255,213,0.75)"}
			menu2.onclose = function(player) vRP.closeMenu({player}) end -- nest menu
						
			menu2["Nej"] = {function(player) vRP.closeMenu({player}) end}
			menu2["Ja"] = {ch_impoundVehicle,"Sælg køretøj på platformen"}
			vRP.openMenu({player,menu2})
		end})
	else
		vRPclient.notify(player,{"Intet køretøj på platformen!"})
	end
end
...
AddEventHandler("vRP:playerSpawn",function(user_id, source, first_spawn)
	...
	vRPclient.addBlip(source,{x,y,z,68,26,"Sælg ødelagte biler"})
...
vRP.registerMenuBuilder({"main", function(add, data)
	...
	if user_id ~= nil then
		...
		if (isPlayerTowTrucker(user_id) == true) then
			choices["Mekaniker menu"] = {ch_towMenu, "Mekaniker menu"}
		end
		add(choices)
	end
end})